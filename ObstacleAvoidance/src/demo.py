...
def get_parser():
    ...
    parser.add_argument("--validasi", action="store_true", help="Untuk validasi dataset")
    parser.add_argument(
        "--output",
...
if __name__ == "__main__":
    ...
    for d in ["train", "val"]:      
        register_coco_instances(f"Components_{d}", {}, f"{objectDir}/{d}.json", f"{objectDir}/{d}")
    ...
    if args.webcam:
        ...
        for d in random.sample(dataset_dicts, 3):  
            ...
            plt.imshow(cv2.cvtColor(visualized_output.get_image()[:, :, ::-1], cv2.COLOR_BGR2RGB))
            plt.show()
